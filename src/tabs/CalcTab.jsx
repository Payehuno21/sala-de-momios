  ...
  return (
    <div className="glass rounded-2xl">
      <div className="flex items-center justify-between px-4 py-3 border-b border-line">
        ...
        {warning && <span className="text-[9px] uppercase tracking-wider px-2 py-0.5 bg-accentSoft text-violet border border-lineGlow">baja confianza</span>}
      </div>
      <div className="px-4">
      ...
      {warning && <p className="text-[11px] leading-relaxed text-violet px-4 py-2 border-t border-line">{warning}</p>}
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-[1fr_60px_70px_64px] items-center gap-2 py-2.5 border-b border-line last:border-b-0">
      ...
      <input
        ...
        className="px-2 py-1.5 text-right tabular-nums text-[13px] font-semibold focus:outline-none w-full glass border border-line text-paper font-mono"
      />
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div className="space-y-4">
        ...
        {candidates.length > 0 && (
          <div className="glass rounded-2xl p-3.5 flex items-center gap-2">
            <span className="text-[12px] whitespace-nowrap text-textDim">Monto a apostar</span>
            <span className="text-[13px] font-bold text-gradient">$</span>
            <input type="number" inputMode="decimal" placeholder="0.00" value={stakeAmount} onChange={e => setStakeAmount(e.target.value)}
              className="flex-1 px-2.5 py-1.5 text-[13px] font-bold tabular-nums focus:outline-none glass border border-line text-paper font-mono" />
          </div>
        )}

        {bestBet && (
          <div className={`rounded-2xl p-4 ${bestBet.ev > 0 ? "glass-strong animate-glow-pulse border-2 border-win/40" : "glass border border-loss/30"}`}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full glass text-textDim">{bestBet.market}</span>
            </div>
            ...
            {bestBet.ev > 0.5 && (
              <div className="text-[11px] mt-2 p-2 rounded-xl bg-accentSoft border border-lineGlow text-violet">
            ...
            <button onClick={() => addToBitacora(bestBet)} disabled={!stakeAmount || parseFloat(stakeAmount) <= 0}
              className="mt-3 w-full py-2 text-[12.5px] font-bold disabled:opacity-30 bg-brand-gradient text-ink rounded-xl transition-transform active:scale-[0.97]">
      ...
      <div className="space-y-4">
        {candidates.length > 0 ? (
              ...
              {candidates.map((c, i) => (
                <div key={i} className="flex items-center justify-between gap-2 py-2.5 border-b border-line last:border-b-0">
                  ...
                  <button onClick={() => addToBitacora(c)} disabled={!stakeAmount || parseFloat(stakeAmount) <= 0}
                    className={`px-3 py-1.5 text-[11px] font-bold whitespace-nowrap disabled:opacity-30 border border-line ${justAdded === c.label ? "bg-win text-ink" : "glass text-paper"}`}>
          ...
          <div className="glass rounded-2xl p-4 text-[12px] text-textDim">
