    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(GROUPS).map(([g, members]) => {
          ...
          return (
            <div key={g} className="border border-line bg-panel">
              <div className="px-3 py-2 border-b border-lineGold text-[11px] uppercase tracking-wider font-bold text-gold">Grupo {g}</div>
              <div className="px-3">
                ...
                {standings.map((s, idx) => (
                  <div key={s.team} className="grid grid-cols-[1.6fr_24px_24px_24px_28px_28px] gap-1 items-center text-[12px] py-1.5 border-b border-line last:border-b-0">
                    <span className="truncate flex items-center gap-1">
                      <span className={`text-[9px] font-bold w-3 ${idx < 2 ? "text-gold" : "text-textDim"}`}>{idx + 1}</span>
              ...
              <div className="flex flex-wrap gap-1.5 px-3 py-2 border-t border-line">
                {[...members].sort((a, b) => (eloTable[b] ?? 1700) - (eloTable[a] ?? 1700)).map(m => (
                  <span key={m} className="text-[9.5px] tabular-nums px-1.5 py-0.5 bg-panel2 text-textDim font-mono">
